function [TV]=traindata(NetA,NetS,M,N,buffer,waittime,trainnum,c,Deepth,MCTSnum,sini,Timehorizon,sigma,tao,raw,QRv,failureprobability)
%lamda=0.9;
discount=0.9;
%%%%%%%%%%%%%%%%%
cvisit=50;
...
for traintime=1:trainnum
    ...
    traindatas=zeros(statenum,1);%观测集合，行长为状态state大小
    traindataa=zeros(actiontype,1);%响应集合，行长为action种类
    traindatav=zeros(QRv,1);%响应集合，value
...
%        for i=1:length(action)
%            [~,www]=process_wt_failure(s,action,M,N,raw,buffer,waittime);
%            Proot(j)=Proot(j)+cprior*mean(www)/M;
%        end
        ...
        while(Mroot>1)
            for mchoose=1:length(action_on_root)
                for helving=1:max(1,floor(log(2)*MCTSnum/(Mroot*log(M0))))
                    ...
                    t=0;
                    if time<=(90)
                        [s,wp]=process_raw(s0,action_on_root(mchoose),M,N,raw,buffer,waittime,failureprobability);
                    else
                        [s,wp]=process(s0,action_on_root(mchoose),M,N,raw,buffer,waittime,failureprobability+0.1);
                    end
                    ...
                    %for t=1:min(Deepth,Timehorizon-time+1)
                        ...
                        if time<=(90)
                            [s,wp]=process_raw(s,action(choose),M,N,raw,buffer,waittime,failureprobability);
                        else
                            [s,wp]=process(s,action(choose),M,N,raw,buffer,waittime,failureprobability+0.1);
                        end
                    ...
                    for t=stoppoint:-1:1
                        ...
                        for tt=1:WT(t)
                            v=wp(tt)*ones(1,QRv)/(1.05*N/(1-discount))+discount*v;
                        end
                        %v=WP(t)*ones(1,QRv)/(1.05*N/(1-discount))+discount*v;
                        if Ns(track(t))>0
        ...
        if isKey(hashTable, stateStr)
            ...
            DTB=sort(DTB);
            DTB0=zeros(1,QRv);
            TL=floor(length(DTB)/QRv);
            for dtb=1:QRv
                DTB0(dtb)=mean(DTB((dtb-1)*TL+1:dtb*TL));
            end
            DTB=DTB0;
        end
        ...
        if time<=(Timehorizon/2)
            s0=process_raw(s0,actionchoose,M,N,raw,buffer,waittime,failureprobability);
        else
            s0=process(s0,actionchoose,M,N,raw,buffer,waittime,failureprobability+0.1);
        end
        ...
        if length(find(pa~=0))>1&&max(DTB)>0
            traindatas(:,end+1)=dlarray(statenow','CB');
            traindataa(:,end+1)=dlarray(pa,'CB');
            traindatav(:,end+1:end+wt)=dlarray(RRR,'CB');
        end
